...
#define CURL_HANDLE static_cast<CURL *>(instance().curl_handle_)
    ...
    */
    ...
    std::string service::send_post(const std::string& endpoint, const request_params& params) {
        ...
        curl_easy_setopt(CURL_HANDLE, CURLOPT_READDATA, &request_body_info);

        // Set content-type header
        header = curl_slist_append(header, MAKE_HEADER("Content-Type", "application/x-www-form-urlencoded"));
    ...
    size_t service::read_callback2(void *data, size_t size, size_t nmemb, void *userdata) {
        // Get upload struct
